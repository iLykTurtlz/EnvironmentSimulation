...
public class PerlinNoiseLandscapeGenerator {
    ...
    public static double[][] generatePerlinNoiseLandscape ( int dxView, int dyView, double scaling, double landscapeAltitudeRatio, int perlinLayerCount)
    {
        ...
        double random_generation = Math.random() * 10 + 1; //allows different generations since perlin noise is based on noise based on fixed primal numbers
    	...
    	for (int x = 0; x < dxView; x++) {
    		for (int y = 0; y < dyView; y++) {
                landscape[x][y] = perlinNoise2D((double) x / dxView * random_generation, (double) y / dyView * random_generation, 1d, perlinLayerCount); //low persistence for a smoother landscape (argument after random_generation (0.4d seems stable now)
            }
        }
